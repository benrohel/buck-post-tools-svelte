<script lang="ts">
  ...
  function handleThumbnailHover(easing: EasingItem, isHovering: boolean): void {
    // This function is now handled within EasingCard component
  }
...
<div class="easing-list">
  {#each filteredEasings as easing, index (easing.name)}
    <EasingCard
      {easing}
      id={index}
      selected={selectedEasing?.name === easing.name}
      onSelect={selectEasing}
