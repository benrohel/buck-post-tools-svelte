<script lang="ts">
  ...
  import { USER_AME_PRESETS } from '@/api/files/files';
  ...
  onMount(async () => {
    const renderSettings = await USER_AME_PRESETS('24.0');
...
{/if}
<div class="flex-row-between">
  <p class="select-label">Select Render Settings:</p>
  <Select
    --width="auto"
    listOffset={2}
    items={renderSettingsList}
    placeholder="Select Render Settings"
    showChevron
    clearable={false}
    listAutoWidth={false}
    justValue
